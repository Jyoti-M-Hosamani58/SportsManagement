...
from .models import Enquiry, AddRegister, Duration, coachReg, Coach_allocation, KitItem, Item_entry, kit_distribution, \
    ...
    Attendence, Coach_Attendence,StockAdjustment, Increasestock, DecreaseStock

from django.core.files.storage import FileSystemStorage
import os
from my_sports.settings import BASE_DIR
...
def register(request):
    ...
    coaches_queryset = coachReg.objects.all()
    coaches = list(coaches_queryset.values('coachname', 'coachType'))
    return render(request, 'Admission/admission.html', {'coaches': coaches})

def get_coach_name(request):
    query = request.GET.get('query', '')
    if query:
        coachname = coachReg.objects.filter(coachname__icontains=query).values_list('coachname', flat=True)
        print('coachname numbers:', list(coachname))  # Debugging: check the data in the terminal
        return JsonResponse(list(coachname), safe=False)
    return JsonResponse([], safe=False)


def get_coach_details(request):
    name = request.GET.get('name', '')
    if name:
        consignor = coachReg.objects.filter(coachname=name).first()
        if consignor:
            data = {
                'coachType': consignor.coachType,
...
def get_coach_type(request, coach_id):
    try:
        ...
        return JsonResponse({'coachType': coach.coachType})  # Assuming 'coachType' is the field name
    except coachReg.DoesNotExist:
        return JsonResponse({'error': 'Coach not found'}, status=404)
def get_enquiry_name(request):
    query = request.GET.get('query', '')
    if query:
        # Perform a case-insensitive search for names starting with the query
        results = Enquiry.objects.filter(name__icontains=query).values_list('name', flat=True)
        return JsonResponse(list(results), safe=False)
    else:
        return JsonResponse([])  # Return an empty list if no query is provided

# 2. View for fetching detailed information about an enquiry
def get_enquiry_details(request):
    name = request.GET.get('name', '')
    if name:
        try:
            enquiry = Enquiry.objects.get(name=name)
            # Return a dictionary of the relevant enquiry details
            data = {
                ...
                'parent_mobile_no': enquiry.parent_mobile_no,
                'place': enquiry.place,
                'email': enquiry.email
            }
            return JsonResponse(data)
...
def get_session_price(request):
    if request.is_ajax() and request.method == "GET":
        package = request.GET.get('package')
        weekly_sessions = request.GET.get('weekly_sessions')

        # Fetch the relevant data from the database
        try:
            # Assuming package is stored in the 'session' field and weekly_sessions in 'weekly_sessions'
            duration = Duration.objects.get(session=package, weekly_sessions=weekly_sessions)
            response = {
                'payment': str(duration.payment),  # send payment as a string to avoid float formatting issues
            }
        except Duration.DoesNotExist:
            response = {
                'error': 'Session/Package combination not found.',
            }
...
def dist_receipt(request, receipt):
    # Fetch all sale records related to the given receipt number
    sale_details = Sale.objects.filter(receipt=receipt)

    # Ensure that the sale details exist for the receipt
    if sale_details.exists():
        # Assuming the first record has player details (as you want player info separately)
        player_details = sale_details.first()  # Get the first Sale record

        # Extract player-specific details
        player_info = {
            'admission_no': player_details.admission_no,
            'name': player_details.name,
            'phone': player_details.phone,
            'address': player_details.address,
            'aadhar': player_details.adhar,
            'place': player_details.place,
            'balance': player_details.balance,
            'package': player_details.package,
            'sessions': player_details.sessions,
            'batchtime': player_details.batchtime,
            'receipt': player_details.receipt,
            'balance': player_details.balance,
        }

        # Calculate the grand total of the totalCost from all sale records
        grand_total = sale_details.aggregate(Sum('totalCost'))['totalCost__sum']

        # Pass player_info, sale_details, and grand_total to the template
        return render(request, 'stock/dist_receipt.html', {
        ...
        # If no sale details exist, return an error message
        return render(request, 'stock/dist_receipt.html', {'error': 'No sale records found for this receipt.'})
...
def get_item_name(request):
    query = request.GET.get('query', '')
    if query:
        # Filter the kit names from the Store model that contain the query string
        kit_names = Store.objects.filter(kit_name__icontains=query).values_list('kit_name', flat=True)
        print('kit_names:', list(kit_names))  # Debugging: check the data in the terminal
        return JsonResponse(list(kit_names), safe=False)
    return JsonResponse([], safe=False)
...
def get_item_details(request):
    ...
    if name:
        try:
            product = Store.objects.get(kit_name=name)
            data = {
                'id': product.id,
...
def get_name(request):
    ...
    if query:
        # Filter the kit names from the Store model that contain the query string
        name = AddRegister.objects.filter(name__icontains=query).values_list('name', flat=True)
        print('kit_names:', list(name))  # Debugging: check the data in the terminal
        return JsonResponse(list(name), safe=False)
    return JsonResponse([], safe=False)


def get_name_details(request):
    name = request.GET.get('name', '')
    if name:
        try:
            player = AddRegister.objects.get(name=name)

            data = {
                'admission_no': player.admission_no,
                'dob': player.dob,
                'name': player.name,
                'phone': player.phone,
                'aadhar': player.adhar,
...
def get_customer(request):
    ...
    if query:
        # Filter the kit names from the Store model that contain the query string
        name = Customer.objects.filter(full_name__icontains=query).values_list('full_name', flat=True)
        print('kit_names:', list(name))  # Debugging: check the data in the terminal
        return JsonResponse(list(name), safe=False)
    return JsonResponse([], safe=False)
...
def get_customer_details(request):
    ...
    if name:
        try:
            player = Customer.objects.get(full_name=name)
...
def product_dist(request):
    if request.method == 'POST':
        ...
        # Get the product details from the POST request (should be JSON)
        products_data = request.POST.get('products')

        print('Products received:', products_data)

        # Parse the JSON data
        try:
            products = json.loads(products_data)  # Safely parse the JSON string into a Python list
        except json.JSONDecodeError:
            print("Error decoding JSON.")
            return render(request, 'stock/product_dist.html', {'error': 'Error decoding product data.'})

        # Check if products exist
        if not products:
            print("No products added.")
            return render(request, 'stock/product_dist.html', {'error': 'No products selected'})

        # Save the player details into the Sale table
        ...
        # Save each product associated with the player
        for product in products:
            # Deduct quantity from Store table
            store_item = get_object_or_404(Store, id=product['product_id'])

            if store_item.quantity < product['quantity']:
                return render(request, 'stock/product_dist.html', {
                    'error': f"Insufficient stock for {store_item.kit_name}. Available: {store_item.quantity}, Requested: {product['quantity']}"
                })

            # Update the Store table
            store_item.quantity -= product['quantity']
            store_item.save()

            # Save the sale record
            Sale.objects.create(
                sale=sale,  # Link this product to the created sale
        ...
        # Commit the transaction and redirect to receipt view
        print(f"Sale completed successfully. Receipt ID: {receipt}")  # Debugging line
        return redirect('dist_receipt', receipt=receipt)
...
def save_sale(request):
    if request.method == 'POST':
        try:
            # Parse player details from the request
            player_details = json.loads(request.POST.get('playerDetails'))
            products = json.loads(request.POST.get('products'))
            receipt_ID = random.randint(1111, 9999)
            receipt = str(receipt_ID)

            # Calculate total cost for all products
            grand_total = sum(float(product['total_cost']) for product in products)

            # Calculate updated balance (subtract total cost from current balance)
            current_balance = float(player_details['balance'])  # Assuming the balance is a number
            updated_balance = current_balance + grand_total  # Deduct the total cost from balance

            # Start a transaction to ensure atomicity
            with transaction.atomic():
                # Save player and product details, and update the stock in Store table
                for product in products:
                    # Save the product details into Sale table
                    sale = Sale(
                        name=player_details['player_name'],
                        admission_no=player_details['admission_no'],
                        phone=player_details['phone'],
                        adhar=player_details['aadhar'],
                        place=player_details['place'],
                        address=player_details['address'],
                        package=player_details['package'],
                        sessions=player_details['sessions'],
                        ...
                        quantity=product['quantity'],
                        price_per_unit=product['price_per_unit'],
                        updatedBalance=updated_balance,  # Updated balance after deduction
                        totalCost=product['total_cost'],
                        receipt=receipt,  # Randomly generated receipt number
                        kit_name=product['product_name'],
                        productname=product['product_name'],  # Assuming you're saving the product name
                        balance=player_details['balance'],  # Save the updated balance
                    )
                    sale.save()

                    # Update the stock in the Store table
                    store_product = Store.objects.filter(id=product['id']).first()

                    if store_product:
                        # Check if enough stock is available
                        if store_product.quantity >= product['quantity']:
                            # Deduct the quantity from Store stock
                            store_product.quantity -= product['quantity']
                            store_product.save()
                        else:
                            # Not enough stock available
                            raise Exception(f"Not enough stock for {product['product_name']}. Available: {store_product.quantity}, Needed: {product['quantity']}.")
                    else:
                        # Product not found in store
                        raise Exception(f"Product {product['product_name']} not found in store.")

            # Return a JSON response indicating success and the receipt number
...
def save_sale_customer(request):
    if request.method == 'POST':
        try:
            # Parse player details from the request
            player_details = json.loads(request.POST.get('playerDetails'))
            ...
            # Set the updated balance (total cost is already deducted)
            updated_balance = grand_total  # Deduct the total cost from balance

            # Start a transaction to ensure atomicity
            with transaction.atomic():
                # Save player and product details, and update the stock in Store table
                for product in products:
                    # Save the product details into Customer_sale table
                    sale = Customer_sale(
                        full_name=player_details['full_name'],
                        phone_number=player_details['phone_number'],
...
def customer_dist_receipt(request, receipt):
    # Fetch all sale records related to the given receipt number
    sale_details = Customer_sale.objects.filter(receipt=receipt)
    ...
    if sale_details.exists():
        ...
        # Pass player_info, sale_details, and grand_total to the template
        return render(request, 'stock/customer_dist_receipt.html', {
            'player_info': player_info,
            'sale_details': sale_details,
            'grand_total':grand_total
        })
        ...
        # If no sale details exist, return an error message
        return render(request, 'stock/customer_dist_receipt.html', {'error': 'No sale records found for this receipt.'})
...
def payment_list(request):
    # Get the search filters from the GET parameters
    admission_no = request.GET.get('admission_no', '')
    name = request.GET.get('name', '')
    start_date = request.GET.get('start_date', '')
    end_date = request.GET.get('end_date', '')

    # Filter payments based on the search criteria
    payments = AddRegister.objects.all()

    if admission_no:
        payments = payments.filter(admission_no__icontains=admission_no)
    if name:
        payments = payments.filter(name__icontains=name)
    if start_date:
        payments = payments.filter(date__gte=start_date)
    if end_date:
        payments = payments.filter(date__lte=end_date)

    return render(request, 'Admission/payment_history.html', {'payments': payments})
...
def stock_adjustment(request):
    if request.method == 'POST':
        ...
        # Basic validation
        if product_name and description and quantity.isdigit():
            quantity = int(quantity)  # Ensure quantity is an integer

            # Create a new StockAdjustment entry
            StockAdjustment.objects.create(
                product_name=product_name,
                description=description,
                quantity=quantity
            )
            return redirect('stock_adjustment')  # Redirect to success page
        else:
            # If validation fails, show an error message
            error_message = "Please fill in all fields correctly."
            return render(request, 'stock_adjustment.html', {'error_message': error_message})
...
def increaseStock(request):
    if request.method == 'POST':
        ...
        if product_name and description and quantity.isdigit():
            ...
            # Check if the combination of company_name and kit_name exists in the Store table
            existing_item = Store.objects.filter(company_name=company_name, kit_name=product_name).first()

            if existing_item:
                # If item exists, update the quantity and price_per_unit
                existing_item.quantity += quantity  # Add the new quantity to the existing one
                existing_item.price_per_unit = price_per_unit  # Update the price per unit
                existing_item.save()  # Save the changes
            else:
                # If the item doesn't exist, create a new entry in the Store table
                Store.objects.create(
                    kit_name=product_name,
                    quantity=quantity,
            ...
            # Optionally, you can create a StockAdjustment entry for record keeping
            Increasestock.objects.create(
                kit_name=product_name,
                description=description,
                quantity=quantity,
                company_name=company_name,
            ...
            return redirect('increaseStock')  # Redirect to the page after success
        else:
            # If validation fails, show an error message
            error_message = "Please fill in all fields correctly."
            return render(request, 'stock/increaseStock.html', {'error_message': error_message})
    companies = Company.objects.all()

    return render(request, 'stock/increaseStock.html',{'companies':companies})

def decreaseStock(request):
    if request.method == 'POST':
        product_name = request.POST.get('productname')
        description = request.POST.get('description')
        quantity = request.POST.get('quantity')
        ...
        if product_name and description and quantity.isdigit():
            ...
            # Redirect after successful processing
            return redirect('decreaseStock')  # Redirect to the page after success
        else:
            # If validation fails, show an error message
            error_message = "Please fill in all fields correctly."
            return render(request, 'stock/decreaseStock.html', {'error_message': error_message})
    companies = Company.objects.all()

    return render(request, 'stock/decreaseStock.html',{'companies':companies})


def increaseStocklist(request):
    # Check if a search term is provided in the request
    product_name = request.GET.get('product_name', '')

    if product_name:
        # Filter the items based on the product_name (case-insensitive search)
        items = Increasestock.objects.filter(kit_name__icontains=product_name)
    else:
        # If no search term, get all items
        items = Increasestock.objects.all()

    return render(request, 'stock/increaseStocklist.html', {'items': items, 'product_name': product_name})

def decreaseStocklist(request):
    # Check if a search term is provided in the request
    product_name = request.GET.get('product_name', '')

    if product_name:
        # Filter the items based on the product_name (case-insensitive search)
        items = DecreaseStock.objects.filter(kit_name__icontains=product_name)
    else:
        # If no search term, get all items
        items = DecreaseStock.objects.all()
    return render(request,'stock/decreaseStocklist.html',{'items':items})